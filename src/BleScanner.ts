...
export class BleScanner extends Emitter {
  constructor(log) {
    super();
    this.log = log;
  ...
  async discoverBleDevices() {
    ...
    this.on('discover', (peripheral) => {
      if (this.devices.has(peripheral.address)) {
        ...
        if (expectedCrc16 !== getCrc16(buffer.slice(0, 5))) {
          ...
          let data = {
            temperature:
							temperature_raw_value >= 0x8000
							  ? (temperature_raw_value - 0x10000) / 100
							  : temperature_raw_value / 100,
